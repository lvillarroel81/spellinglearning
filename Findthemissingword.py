...
"""
Proyecto realizado en  Python (The Hang up)
Basado en el proyecto de: Kylie Ying. 
Versión en inglés con Modificaciones: Luis Manuel Villarroel Carreno.
"""
import random
import string

from Wordlist import Wordlist
from hang_up_diagramas import vidas_diccionario_visual


def obtener_palabra_válida(Wordlist):
    Wordlist = random.choice(Wordlist)  # seleccionar una palabra al azar de la lista
...
def The_Hang_up():
    ...
    Wordlist= obtener_palabra_válida(Wordlist) (Wordlist)
    letras_por_adivinar = set(Wordlist)  # conjunto de letras de la palabra que deben ser adivinadas.
    ...
    while len(letras_por_adivinar) > 0 and vidas > 0:
        ...
        palabra_lista = [letra if letra in letras_adivinadas else '-' for letra in Wordlist]
    ...
    if vidas == 0:
        ...
        print(f"¡Ahorcado! Perdiste. Lo lamento mucho. La palabra era: {Wordlist}")
    else:
        print(f'¡Excelente! ¡Adivinaste la palabra {Wordlist}!')
